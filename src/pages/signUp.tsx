...
export interface formprps {
  name: string;
  email: string;
  password?: string;
  dateOfBirth: string;
  country: SingleValue<{ label: string; value: string }>;
  favSport: SingleValue<{ label: string; value: string }>;
  FavSportTeam: SingleValue<{ label: string; value: string }>;
}
...
//   };
  ...
  return (
    <div
      className={`flex hidden flex-col justify-center items-center py-[2rem] px-[.8rem] text-black rounded-[1rem] bg-[white] w-full scale ${
        visible ? "reset-position" : "scale-down"
      } sm:w-[26rem] sm:py-[2.5rem] sm:px-[3rem]`}
      style={{ position: "absolute", left: "0", marginTop: "-3rem" }}
    >
      <Formik
        initialValues={{ email: "", password: "", name: "", favorite_team: "" }}
        validationSchema={validationSchema}
        onSubmit={async (values, { setSubmitting }) => {
            const queryParams = new URLSearchParams({ email: values.email })
          ...
          try {
            ...
            const response = await axios.post(`${url}/auth/email-signup`, {
            ...
            if (response.status === 200) {
              ...
              navigate(`/auth/verification?${queryParams.toString()}`);
        ...
        {({ isSubmitting, errors, touched }) => (
          <Form>
            ...
            <Field
              type="text"
              name="name"
              className={`w-full h-[40px] pl-3 mb-[.5rem] outline-none border-2 ${
                errors.email && touched.email ? "border-[rgb(190, 63, 13)]" : ""
              } border-[rgb(181, 179, 187)] rounded`}
            ...
            />
            <Field
              className={`w-full h-[40px] pl-3 mb-[.5rem] rounded border-2 ${
                errors && touched
            ...
            <p className="text-[13px] mb-2">
              Your Date of Birth will not appear on your profile. Please confirm
              your real age
            </p>
            <div className="mb-3">
              <CountriesDropDown
            ...
            <div className="mb-4">
              ...
              <Field
                ...
                name="favorite_team"
                className={`w-full h-[40px] pl-3 mb-[.5rem] outline-none border-2 ${
                  errors.favorite_team && touched.favorite_team
                    ? "border-[rgb(190, 63, 13)]"
                    : ""
                } border-[rgb(181, 179, 187)] rounded`}
